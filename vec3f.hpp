...
public:
    ...
    inline vec3f operator*(float scale) const { return vec3f(scale * x, scale * y, scale * z); }
    
	inline vec3f operator*(const vec3f& vec_b){ return vec3f(vec_b.get_x()*x, vec_b.get_y()*y, vec_b.get_z()*z); } 
